...
const updateFilters = () => {
  if (searchText.value.length > 0) {
    ...
    formReferences.htmlPageName.value = searchText.value
    if (!props.filters.some((x) => x.key === 'htmlPageName')) {
      emit('update:filters', [...props.filters, filter])
    } else {
      emit(
        'update:filters',
        props.filters.map((x) => (x.key === 'htmlPageName' ? filter : x))
      )
    }
  } else {
    emit(
      'update:filters',
      props.filters.filter((x) => x.key !== 'htmlPageName')
    )
  }
}
</script>
...
<template>
  <VControl icon="feather:search">
    <input
      ...
      :placeholder="`${t('filter.searchText')} ${t('html.htmlPageName')}`"
