...
const updateFilters = () => {
  if (searchText.value.length > 0) {
    ...
    formReferences.originPolicyName.value = searchText.value
    if (!props.filters.some((x) => x.key === 'originPolicyName')) {
      emit('update:filters', [...props.filters, filter])
    } else {
      emit(
        'update:filters',
        props.filters.map((x) => (x.key === 'originPolicyName' ? filter : x))
      )
    }
  } else {
    emit(
      'update:filters',
      props.filters.filter((x) => x.key !== 'originPolicyName')
    )
  }
}
</script>
...
<template>
  <VControl icon="feather:search">
    <input
      ...
      :placeholder="`${t('filter.searchText')} ${t('originPolicy.ruleName')}`"
