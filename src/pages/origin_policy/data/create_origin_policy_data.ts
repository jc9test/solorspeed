...
// import axiosClient from '/@src/composable/useApi'
...
const loadbalancerSticky = ref(false)
  ...
  for (let kIndex = 0; kIndex < keys.length; kIndex++) {
    ...
    submissionObject[keys[kIndex]] =
      tempVal.filter((item) => item.key == keys[kIndex]).length > 0
        ...
        : formReferences[keys[kIndex]].value
    // submissionObject[keys[kIndex]] = tempVal.filter((item)=>item.key==keys[kIndex]).length>0
    //   ? {value: formReferences[keys[kIndex]].value, error:false}
    //   : formReferences[keys[kIndex]].value
  }
