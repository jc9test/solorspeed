...
import type { SolarspeedAppContext } from './app'
import 'vue-prism-editor/dist/prismeditor.min.css' // import the styles

// Lazy load aditional components
export async function registerGlobalComponents({ app }: SolarspeedAppContext) {
 ...
 * </template>
 */
export function registerRouterNavigationGuards({ router, api }: SolarspeedAppContext) {
  router.beforeEach(async (to, from) => {
    ...
    if (from === START_LOCATION && userSession.isLoggedIn) {
      ...
      //   duration: 7000,
      // })
      notyf.error('Sorry, you should loggin to access this section (anything will work)')
